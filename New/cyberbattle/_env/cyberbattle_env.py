...
# 변경 사항
# 1. toyctf 환경을 위한 조기종료조건 개선 (__attacker_goal_reached)
# 2. throws_on_invalid_actions = False로 변경시 penalty reward 계산 가능
...
from offpolicy.CyberBattleSim_baseline.cyberbattle._env.defender import DefenderAgent
from offpolicy.CyberBattleSim_baseline.cyberbattle.simulation.model import PortName, PrivilegeLevel
...
class EnvironmentBounds(NamedTuple):
    ...
    def of_identifiers(cls,
        ...
        print(f'port: {identifiers.ports}')
        print(f"properties : {identifiers.properties}")
        print(f"local_attacks : {identifiers.local_vulnerabilities}")
        print(f"remote_attacks : {identifiers.remote_vulnerabilities}")
...
class CyberBattleEnv(gym.Env):
    ...
    def __execute_action(self, action: Action) -> actions.ActionResult:
        ...
        if "local_vulnerability" in action:
            ...
            result = self._actuator.exploit_local_vulnerability(
            ...
            result, goal = self._actuator.exploit_remote_vulnerability(
                source_node_id,
                target_node_id,
            ...
            if credential_cache_index < 0 or credential_cache_index >= len(self.__credential_cache):
                return actions.ActionResult(reward=-1, outcome=None), goal
...
#            return False
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, StepInfo]:
        ...
        try:
            result, customer  = self.__execute_action(action)
    ...
    def reset(self) -> Observation:
        ...
        self.__owned_nodes_indices_cache = None
        return observation
#model_path:str, agent_path:str ,i_episode:int
