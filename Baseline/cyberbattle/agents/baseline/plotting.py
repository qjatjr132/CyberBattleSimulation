...
def plot_episodes_length(learning_results):
    ...
    plt.xticks(size=20)
    plt.yticks(size=20)
    plt.title("iterations of each episode", fontsize=12)
...
class PlotTraining:
    ...
    def plot_durations(self, average_window=5):
        ...
        plt.legend(['iteration', 'moving average'])
...
def epidosed_rewards(run):
    max_iteration_count = np.max([len(r) for r in run['reward per episodes']])
    return 
...
def plot_episodes_reward(learning_results):
    """Plot length of every episode"""
    plt.figure(figsize=(10, 8))
    ...
    plt.xticks(size=20)
    plt.yticks(size=20)
    plt.title("Reward of each episode", fontsize=12)

    for results in learning_results:
        rewards = [e for e in results['reward_per_episodes']]
        episode = [i for i in range(len(results['reward_per_episodes']))]
        plt.plot(episode, rewards, label=f"{results['title']}")

    plt.legend(loc="upper right")
    plt.show()


def plot_episodes_reward_each(learning_results):
    """Plot length of every episode"""
    for results in learning_results:
        plt.figure(figsize=(10, 8))
        plt.ylabel('rewards', fontsize=20)
...
def plot_episodes_length_each(learning_results):
    """Plot length of every episode"""
    for results in learning_results:
        plt.figure(figsize=(10, 8))
        plt.ylabel('iterations', fontsize=20)
