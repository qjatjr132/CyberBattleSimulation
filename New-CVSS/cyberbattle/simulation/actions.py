...
class AgentActions:
    ...
    def __init__(self, environment: model.Environment, throws_on_invalid_actions=False):
        """
    ...
    def __process_outcome(self,
        ...
        #State Transition 추가
        if random.random() > node_info.success_rate:
            return False, ActionResult(reward=0,
                                       outcome=None), customerdata
        ...
        if is_global_vulnerability:
            ...
            if throw_if_vulnerability_not_present:
                ...
                return False, ActionResult(reward=Penalty.SUPSPICIOUSNESS, outcome=None), customerdata
        ...
        if not self._check_prerequisites(node_id, vulnerability):
            return False, ActionResult(reward=failed_penalty, outcome=model.ExploitFailed()), customerdata
        ...
        if isinstance(outcome, model.PrivilegeEscalation):
            if outcome.tag in node_info.properties:
                return False, ActionResult(reward=Penalty.REPEAT, outcome=outcome), customerdata
        ...
        return True, ActionResult(reward=reward, outcome=outcome), customerdata
    ...
    def exploit_remote_vulnerability(self,
        ...
        is_global_vulnerability = vulnerability_id in self._environment.vulnerability_library
        is_inplace_vulnerability = vulnerability_id in target_node_info.vulnerabilities

        if is_global_vulnerability:
            vulnerabilities = self._environment.vulnerability_library
        elif is_inplace_vulnerability:
            vulnerabilities = target_node_info.vulnerabilities
        else:
            if throw_if_vulnerability_not_present:
                raise ValueError(f"Vulnerability '{vulnerability_id}' not supported by node='{node_id}'")
            else:
                logger.info(f"Vulnerability '{vulnerability_id}' not supported by node '{node_id}'")
                return ActionResult(reward=Penalty.INVALID_ACTION, outcome=None), customerdata
        ...
        if not source_node_info.agent_installed:
            if self._throws_on_invalid_actions:
                ...
                reward = Penalty.INVALID_ACTION + vulnerability.cost
                return ActionResult(reward, outcome=None), customerdata
        ...
        succeeded, result, customerdata = self.__process_outcome(
    ...
    def exploit_local_vulnerability(self, node_id: model.NodeID,
        ...
        if is_global_vulnerability:
            ...
            if throw_if_vulnerability_not_present:
                ...
                logger.info(f"Vulnerability '{vulnerability_id}' not supported by node '{node_id}'")
                return ActionResult(reward=Penalty.INVAILD_ACTION, outcome=None), customerdata
        ...
        if not node_info.agent_installed:
            if self._throws_on_invalid_actions:
                ...
                reward = Penalty.INVALID_ACTION + vulnerability.cost
                return ActionResult(reward, outcome=None)

        succeeded, result, goal = self.__process_outcome(
...
#시각화 내용 추가
    ...
    # def save_all_attacks(self, model_path:str, agent_path:str ):
    #     df = pd.DataFrame.from_dict(self.list_all_attacks()).set_index('id')
    #     return df        

    # def append_csv(self, df) -> None:
    #     update_df=df.append(pd.DataFrame.from_dict(self.list_all_attacks()).set_index('id'))
    #     return update_df
    
    # def save_csv(self, df, model_path:str, agent_path:str ,i_episode:int)->None:
    #     if not os.path.exists(f'./result/{model_path}/{agent_path}/csv/'):
    #         os.makedirs(f'./result/{model_path}/{agent_path}/csv/')
    #     df.to_csv(f'./result/{model_path}/{agent_path}/csv/episode {i_episode}/save.csv')

    def save_csv(self, path ,i_episode:int, t:int) -> None :
        with open(path, "a") as file:
            file.write(f"========================Time step {t}========================\n")
        df = pd.DataFrame.from_dict(self.list_all_attacks())
