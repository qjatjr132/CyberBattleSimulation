...
class ReplayMemory_DDQN(object):
...
class DuelingDQN(nn.Module):
    ...
    def __init__(self, ep: EnvironmentBounds):
        ...
        ep = CyberBattleStateActionModel(ep)
        #self.state_space = len(ep.state_space.dim_sizes)
        self.state_space = ep.state_space
        self.action_space = ep.action_space

        self.feature_layer = nn.Sequential(
            nn.Linear(len(self.state_space.dim_sizes), 128),
            nn.ReLU()
        )
        ...
        self.advantage_layer = nn.Sequential(
            nn.Linear(128, 128),
            nn.ReLU(),
            nn.Linear(128, self.action_space.flat_size())
        )
...
class DuelingDQNLearnerPolicy(Learner):
    ...
    def __init__(self,
        ...
        self.policy_net = DuelingDQN(ep).to(device)
        self.target_net = DuelingDQN(ep).to(device)
        self.target_net.load_state_dict(self.policy_net.state_dict())
        self.target_net.eval()
        self.target_update = target_update

        self.optimizer = optim.RMSprop(self.policy_net.parameters(), lr=learning_rate)
        self.memory = ReplayMemory_DDQN(replay_memory_size)
    ...
    def optimize_model(self, norm_clipping=False):
        ...
        writer.add_scalar('training loss',loss,int(dir[0]))
        path = f'./result/timestep/{int(dir[0])+1}'
        os.makedirs(path)
