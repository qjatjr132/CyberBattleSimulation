...
class DRQN(nn.Module):
    ...
    def __init__(self, ep: EnvironmentBounds):
        ...
        cnn_input_size = len(model.state_space.dim_sizes)
#        linear_input_size = len(model.state_space.dim_sizes)
        output_size = model.action_space.flat_size()

        self.lstm = nn.LSTM(cnn_input_size, hidden_size = 1024, num_layers = 2 , batch_first = True)
        self.lstm1 = nn.LSTM(1024, hidden_size = 512, num_layers = 2 , batch_first = True)

        # self.bn1 = nn.BatchNorm1d(256)
        # self.hidden_layer4 = nn.Linear(128, 64)
        self.head = nn.Linear(1024, output_size)
    ...
    def forward(self, x):
        ...
        return self.head(x.view(x.size(0), -1)),x
...
class ChosenActionMetadata(NamedTuple):
    """Additonal info about the action chosen by the DRQN-induced policy"""
...
class DeepRQLearnerPolicy(Learner):
    """Deep R Q-Learning on CyberBattle environments
    ...
    def optimize_model(self, norm_clipping=False):
        ...
        output, _ = self.policy_net(state_batch)
    ...
    def lookup_DRQN(self, states_to_consider: List[ndarray]) -> Tuple[List[np.int32], List[np.int32]]:
    ...
    def exploit(self,
        ...
        remaining_action_lookups, remaining_expectedq_lookups = self.lookup_DRQN(candidate_actor_state_vector)
